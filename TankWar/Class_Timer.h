...
public:
	...
	/********根据游戏需求新增********/
	static void initGameTimer();//初始化计时所需的数据0
	static void GameSleep(int ms);//游戏专用延迟（参数代表游戏是否处于暂停状态，false表示没暂停）
	static DWORD GetGameTime();//获取游戏实际经过的毫秒数
	static void renewSysClk();//刷新游戏循环开始前的系统周期数
};